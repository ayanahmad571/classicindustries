...
Stilewell General Trading LLC<br>
	...
	echo '<!-- brand -->
            <div class="logo">
				...
				<a href="#" class="logo-expanded">
                    <img style="align-self:center;width:110px !important"  src="img/logo.png" class="img-responsive" />
                </a>
            </div>
            <!-- / brand -->';
}
...
<?php
        ...
        <meta name="description" content="StileWell">
...
function make_company_ar($conn){
	
	if(1==1){
		$USER = array();

	
$sql= "SELECT * FROM `sw_master_company` WHERE 1";
	$sres = $conn->query($sql);
	##
	if($sres->num_rows ==1){
		while($srow = $sres->fetch_assoc()){
		...
		die("#ERRORINC4452<br>
(Company Not Found <a href='logout.php'><button>Re-login</button></a>)");
	}
	##
	}else{
		return false;
...
function getIndianCurrency($number)
{
    ...
    $digits_length_decimal = strlen($decimal);
    $i = 0;
    $str = array();
    $stra = array();
...
$i=0;
    ...
    while( $i < $digits_length_decimal ) {
        $divider = ($i == 2) ? 10 : 100;
        $number = floor($decimal % $divider);
        $no = floor($decimal / $divider);
        $i += (($divider == 10) ? 1 : 2);
		$plural = (($counter = count($stra)) && $number > 9) ? '' : null;
        if ($number) {
            $stra [] = ($number < 21) ? $words[$number].' '. $digits[$counter].' '.$hundred:$words[floor($number / 10) * 10].' '.$words[$number % 10]. '  ';
        } else $stra[] = '';
    }
    $paise = implode('', array_reverse($stra));


    $paise = ($decimal) ? " and " . ($paise) . ' Paise' : '';
